...
let checkIngredients = (inventory, newRecipt, castList) => {
    let needCast = null
    console.error('checkIngredients', newRecipt)
    needCast = inventory.findIndex((inv, index) => (inv + newRecipt[index] < 0))
    console.error('needCast index', needCast) // 2
    if (needCast !== -1) {
        // find quickest way to cast number 2
...
while (true) {
    ...
    let avgSteps = totalSteps / brewList.length;
    ...
    // }
    let newReciptObject = brewListSort.find((el) => (el.steps <= Math.ceil(avgSteps)))
    newRecipt = newReciptObject.delta
    console.error('newRecipt avgSteps, high price', newReciptObject, newReciptObject.delta ) 
    ...
    // if (freeLearn) {
    //     enoughIngredients = inventoryList.some((item, i) => 
    //         freeLearn.deltaLearn[i] < 0 && (item + freeLearn.deltaLearn[i] < 0)
    //     )
    // }
    if (freeLearn && freeLearn.tomeIndex === 0) {
        castList.push(freeLearn.deltaLearn)
        console.log('LEARN', freeLearn.actionId)
    } else if (freeLearn && freeLearn.tomeIndex === 1 && inventoryList[0] > 0) {
        castList.push(freeLearn.deltaLearn)
        console.log('LEARN', freeLearn.actionId)
